...
class Converter:
    ...
    def tf_to_onnx(
            ...
            only_weigths_of_model: Literal['DenseNet121', 'DenseNet201', None] = None,
            opset: int = 13
    ) -> onnx.ModelProto:
        """
        ...
        try:
            ...
            if output_path:
                ...
                self._validate_onnx_file(output_path)
            else:
                self._validate_onnx_model(onnx_model)  # Fixed: use model validation for in-memory models
    ...
    def _validate_onnx_file(self, onnx_path: Path) -> bool:
        """
        Validate ONNX model structure from file.
    ...
    def _validate_onnx_model(self, onnx_model: onnx.ModelProto) -> bool:
        """
        Validate ONNX model structure from ModelProto object.

        Args:
            onnx_model: ONNX ModelProto object
    ...
    def onnx_to_trt(
        ...
        try:
            if isinstance(input_model, (str, os.PathLike)):
                self.logger.info(f"Loading ONNX file: {input_model}")
                if not os.path.exists(input_model):
                    raise FileNotFoundError(f"ONNX file not found: {input_model}")
                onnx_path = str(input_model)

            elif isinstance(input_model, onnx.ModelProto):
                self.logger.info("Loading ONNX model from ModelProto object")
                # Save the in-memory model to a temporary file for TensorRT
                with tempfile.NamedTemporaryFile(suffix='.onnx', delete=False) as temp_file:
                    onnx.save(input_model, temp_file.name)
                    onnx_path = temp_file.name
                    temp_onnx_path = temp_file.name  # Remember to clean up
            else:
                raise TypeError(
                    f"input_model must be either a string (file path) or onnx.ModelProto object, "
            ...
            if engine_file_path is None:
                if auto_generate_engine_path:
                    if temp_onnx_path:
                        # For temporary files, use a generic name
                        engine_file_path = Path("model.trt")
                    else:
                        engine_file_path = Path(onnx_path).with_suffix('.trt')
            ...
            if temp_onnx_path and os.path.exists(temp_onnx_path):
                ...
                self.logger.info(f"Cleaned up temporary ONNX file: {temp_onnx_path}")
